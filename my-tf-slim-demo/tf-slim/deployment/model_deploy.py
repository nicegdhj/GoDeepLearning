...
To easily train_data a model on multiple GPUs or across multiple machines this
...
Usage:
  ...
  with tf.device(config.inputs_device()):
    maybe_images, labels = LoadData(...)
    inputs_queue = slim.data.prefetch_queue((maybe_images, labels))

  # Define the optimizer.
  with tf.device(config.optimizer_device()):
    optimizer = tf.train_data.MomentumOptimizer(FLAGS.learning_rate, FLAGS.momentum)

  # Define the model including the loss.
  def model_fn(inputs_queue):
    maybe_images, labels = inputs_queue.dequeue()
    predictions = CreateNetwork(maybe_images)
  ...
  # Run training.
  slim.learning.train_data(model_dp.train_op, my_log_dir,
...
DeployedModel namedtuple, holds together the values needed to train_data multiple
clones:
